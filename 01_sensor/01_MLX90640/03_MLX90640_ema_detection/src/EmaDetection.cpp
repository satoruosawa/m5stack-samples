...
void EmaDetection::Setup() {
  ...
  // to get complete picture MLX90640_SetRefreshRate(mlx90640Address, 0x00);
  // //Set rate to 0.25Hz effective - Works
  // MLX90640_SetRefreshRate(mlx90640Address, 0x01); //Set rate to 0.5Hz
  // effective - Works MLX90640_SetRefreshRate(mlx90640Address, 0x02); //Set rate
  // to 1Hz effective - Works MLX90640_SetRefreshRate(mlx90640Address, 0x03);
  // //Set rate to 2Hz effective - Works
  // MLX90640_SetRefreshRate(mlx90640Address, 0x04); //Set rate to 4Hz effective
  // - Works
  MLX90640_SetRefreshRate(mlx90640Address,
                          0x05);  // Set rate to 8Hz effective - Works at 800kHz
  // MLX90640_SetRefreshRate(mlx90640Address, 0x06); //Set rate to 16Hz
  // effective - Works at 800kHz MLX90640_SetRefreshRate(mlx90640Address, 0x07);
  // //Set rate to 32Hz effective - fails
...
void EmaDetection::UpdateSample() {
  for (int i = 0; i < VALUE_COUNT; i++) {
    ...
    if (value < 1 || value > 100) return;
    sampleEmaValues[i] =
        DoubleLerp(0, sampleEmaValues[i], 1, value, sampleEmaCoef);
  }
}
